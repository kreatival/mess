...
public class ServerLicenseUtil {
	...
	try (LicenseReader reader = new LicenseReader(filepath + "/" + Constants.serverLicenseFilename)) {
		...
		if (licensedUsers >= actualUsers || actualUsers <= 10) {
	    ...
	    if (actualUsers <= 10) {
		return new Pair<LicenseStatus, Pair<Integer, Integer>>(LicenseStatus.OK, new Pair<Integer, Integer>(10, actualUsers));		
